...
func (toolbaktring *String)FirstLetter(str string) string {
	if str != "" {
		// 获取字符串第一个字符
		_, size := utf8.DecodeRuneInString(str)
		firstChar := str[:size]

		if toolbaktring.IsLetters(firstChar) {
			return firstChar
		} else if regexp.MustCompile("^[\u4e00-\u9fa5]+$").MatchString(firstChar) {
			// Utf8 转 GBK2312
			firstCharGbk, _ := toolbaktring.Utf8ToGbk(firstChar)
